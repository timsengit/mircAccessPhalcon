...
class MyPaginator
{
    ...
    private function setPage()
    {
        if (!empty($_GET['pagination'])) {
            if ($_GET['pagination'] > 0) {
                if ($_GET['pagination'] > $this->pagenum) {
                    return $this->pagenum;
                } else {
                    return $_GET['pagination'];
                }
            } else {
                return 1;
            }
        ...
        if ($total <= 10) {
            for ($i = 1; $i <= $total; $i++) {
                if ($i == $page) {
                    ...
                    $_pagelist .= '<li><a href="' . $this->url . 'pagination=' . $i . '">' . $i . '</a></li>';
                }
            }
    ...
    private function first()
    {
        ...
        if ($page > 1) {
            ...
            $_pagelist .= '<li class="previous"><a href="' . $this->url . 'pagination=' . $prePage . '">&laquo;</a></li>';
        }
        return $_pagelist;
    }
    ...
    private function last()
    {
        ...
        $_pagelist = '';
        if ($page < $total) {
            $_pagelist .= '<li class="next"><a href="' . $this->url . 'pagination=' . $nextPage . '">&raquo;</a></li>';
